...
Prism Palette Extractor (“Prism”, “the extension”) is a Chrome extension that helps designers extract color palettes from web pages. This policy describes what the extension accesses and how that information is handled.

## Summary

- Prism runs **entirely in your browser**. Palette extraction and curation do not send page data to our servers.
...
- **Rendered page styles** (for example background, text, and border colors) on the tab you selected, only while you run **Extract palette**.
...
- No transmission of extracted palettes or raw page samples to Prism developers or third-party backends operated by us.

## Network connections

**Palette extraction** does not require network access and does not upload page content.
...
| `scripting` | Inject a short-lived script to sample colors on that tab |
| `clipboardWrite` | Copy the generated palette image when you click **Copy palette** |
...
Extracted palettes exist only in extension memory and your clipboard until you close the panel or copy elsewhere. Prism does not store palettes on remote servers.
